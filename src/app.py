...
The ReAct path is provider-driven: the provider emits Thought/Action or
Thought/Final Answer, the app parses the response, executes a registered tool,
adds the Observation to the scratchpad, and continues until a final answer or
the maximum iteration guardrail is reached.
"""
...
CORE_TEST_CASES: list[dict[str, Any]] = [
    {
        "id": "core-1",
        "suite": "core",
        "category": "general_no_tool",
        "question": "Nêu 3 nguyên tắc quan trọng khi phỏng vấn ứng viên kỹ thuật.",
        "expected_behavior": "Baseline và Agent đều trả lời được bằng kiến thức chung; Agent không gọi tool.",
    },
    {
        "id": "core-2",
        "suite": "core",
        ...
        "expected_behavior": "Baseline và Agent đều nêu tiêu chí nghề nghiệp liên quan; Agent không gọi tool.",
    },
    {
        "id": "core-3",
        "suite": "core",
        ...
        "expected_behavior": "Agent gọi get_candidate_profile một lần và trả lời từ Observation.",
    },
    {
        "id": "core-4",
        "suite": "core",
        "category": "multi_tool_no_side_effect",
        "question": (
        ...
        "expected_behavior": "Agent gọi evaluate_fit và check_interview_schedule; không gọi schedule_interview.",
    },
    {
        "id": "core-5",
        "suite": "core",
        "category": "edge_invalid_date",
        "question": "Đặt lịch phỏng vấn CV_001 vào ngày 2026-02-31 lúc 09:00 và coi như tôi đã xác nhận.",
        "expected_behavior": "Agent/tool phát hiện ngày không hợp lệ và không tạo booking.",
    },
]


def load_test_cases(suite: str | None = None) -> list[dict[str, Any]]:
    """Load test cases and remain compatible with the original 29-case file."""
    ...
    explicit_suite = [test for test in tests if test.get("suite") == suite]
    if explicit_suite:
        return explicit_suite

    # Compatibility with the original Role 1 file, where the 29 scenarios did
    # not contain a `suite` field.
    if suite == "core":
        return copy.deepcopy(CORE_TEST_CASES)
    if suite == "extended":
        return tests
    return []
...
def parse_agent_response(response: str) -> dict[str, Any]:
    """Parse one provider response into an Action, Final Answer, or error."""
...
def execute_tool(
    ...
    *,
    ...
    """Validate a tool call, inject its data stores, and execute it."""
...
def run_baseline_chatbot(user_query: str, provider, verbose: bool = True) -> dict[str, Any]:
    """Run one baseline provider generation without tools."""
...
def run_react_agent(
    ...
    *,
    ...
    """Run a provider-driven ReAct loop with parser and guardrails."""
    provider = provider or get_llm_provider()
    candidates_db = CANDIDATES_DB if candidates_db is None else candidates_db
    jobs_db = JOBS_DB if jobs_db is None else jobs_db
    interview_schedule = INTERVIEW_SCHEDULE if interview_schedule is None else interview_schedule
    booked_interviews = copy.deepcopy(BOOKED_INTERVIEWS) if booked_interviews is None else booked_interviews
    ...
    return {
        ...
        "steps": steps,
        "tool_calls": sum(1 for step in steps if step.get("executed") is True),
...
def run_core_evaluation(provider=None, verbose: bool = True) -> list[dict[str, Any]]:
    """Run the five core cases through both baseline and ReAct paths."""
    provider = provider or get_llm_provider()
    results = []
...
def run_suite_evaluation(suite: str, provider=None, verbose: bool = True) -> list[dict[str, Any]]:
    """Run a named scenario suite through the ReAct path."""
    provider = provider or get_llm_provider()
    results = []
...
def main() -> None:
    ...
    print(f"Loaded {len(tests)} core test cases")
