...
class AgentTests(unittest.TestCase):
    ...
    def test_repeated_action_not_executed_twice(self):
        result = run_react_agent("[TEST_REPEATED_ACTION] Hãy lấy hồ sơ CV_001 nhưng đừng lặp vô hạn.", self.provider, verbose=False)
        ...
        executed_profile_calls = [
            step for step in result["steps"] if step.get("tool_name") == "get_candidate_profile" and "REPEATED_ACTION" not in step.get("observation", "")
        ]
        self.assertEqual(len(executed_profile_calls), 1)

    def test_max_iterations_safe_fallback(self):
        result = run_react_agent("[TEST_MAX_ITERATIONS] Hãy tiếp tục sinh lỗi để kiểm tra guardrail.", self.provider, verbose=False)
    ...
    def test_mock_provider_react_path_calls_provider_through_true_loop(self):
        result = run_react_agent("CV_001 có bao nhiêu năm kinh nghiệm và có những kỹ năng nào?", self.provider, verbose=False)
